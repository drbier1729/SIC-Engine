...
int ClipSegmentToLine(ClipVertex vOut[2], ClipVertex vIn[2],
					  const sic::vec2& normal, float offset, char clipEdge)
{
...
static void ComputeIncidentEdge(ClipVertex c[2], const sic::vec2& h, const sic::vec2& pos,
								const Mat22& Rot, const sic::vec2& normal)
{
	// The normal is from the reference box. Convert it
	// to the incident boxe's frame and flip sign.
	Mat22 RotT = Rot.Transpose();
	sic::vec2 n = -(RotT * normal);
	sic::vec2 nAbs = Abs(n);
	...
	if (nAbs.x > nAbs.y)
	{
		if (Sign(n.x) > 0.0f)
		{
			c[0].v = sic::vec2(h.x, -h.y);
			c[0].fp.e.inEdge2 = EDGE3;
			c[0].fp.e.outEdge2 = EDGE4;

			c[1].v = sic::vec2(h.x, h.y);
...
int Collide(Contact* contacts, sic::Body* bodyA, sic::Body* bodyB)
{
	// Setup
	sic::vec2 hA = 0.5f * bodyA->width;
	sic::vec2 hB = 0.5f * bodyB->width;

	sic::vec2 posA = bodyA->position;
	sic::vec2 posB = bodyB->position;
	...
	sic::vec2 dp = posB - posA;
	sic::vec2 dA = RotAT * dp;
	sic::vec2 dB = RotBT * dp;
	...
	// Box A faces
	sic::vec2 faceA = Abs(dA) - hA - absC * hB;
	if (faceA.x > 0.0f || faceA.y > 0.0f)
		return 0;

	// Box B faces
	sic::vec2 faceB = Abs(dB) - absCT * hA - hB;
	...
	sic::vec2 frontNormal, sideNormal;
